"""Validated ML benchmark pipeline for MTBLS28 LC-MS data."""
...
def _ensure_dataframe(X: pd.DataFrame | np.ndarray) -> pd.DataFrame:
    """Convert input to DataFrame while preserving column names when possible."""
...
def _encode_binary_target(y: pd.Series, positive_label: str) -> pd.Series:
    """Encode labels to binary: positive_label=1, Control=0."""
    y_str = y.astype(str)
...
class SampleNormalizer(BaseEstimator, TransformerMixin):
    """Sample-wise normalization for metabolomics tables."""
...
class ParetoScaler(BaseEstimator, TransformerMixin):
    """Pareto-like scaling: center then divide by sqrt(std)."""
...
class TensorFlowMLPClassifier(BaseEstimator, ClassifierMixin):
    """sklearn-compatible TensorFlow MLP for binary classification."""
    ...
    def _compute_class_weight(self, y_np: np.ndarray) -> dict[int, float] | None:
        ...
        if n_pos == 0 or n_neg == 0:
            return None
        return {
            0: float(n_all / (2.0 * n_neg)),
            1: float(n_all / (2.0 * n_pos)),
        }
...
def make_train_valid_test_split(
    ...
    """Create stratified 70/15/15 split with DataFrame feature-name preservation."""
...
def _build_preprocess_pipeline(
    ...
    """Build leakage-safe preprocess pipeline."""
    if scaler == "standard":
...
def _build_models(random_state: int = 42) -> dict[str, BaseEstimator]:
...
def evaluate_binary_classifier(
    ...
    """Compute binary classification metrics and return prediction rows."""
...
def extract_model_features(
    ...
    """
    Extract top features from fitted model pipeline.

    Returns columns:
      model, feature, importance, rank, selection_method
    """
...
def plot_metric_comparison(metrics_df, metric="roc_auc", output_path=None):
    """Plot validation vs test metric comparison across models."""
    df = metrics_df.copy()
...
def plot_roc_curves(predictions_df, output_path=None):
    """Plot ROC curves from prediction table."""
...
def plot_confusion_matrices(metrics_df, output_dir=None):
    """Plot confusion matrices for each model/dataset row in metrics."""
    figs = []
...
def run_ml_benchmark(
    ...
    """
    Run leakage-safe benchmark with feature selection and model comparison.

    Returns dict with:
      metrics, predictions, best_estimators, selected_features, split_info
    """
    X_df = _ensure_dataframe(X)
    y_s = _ensure_series(y, index=X_df.index)
    ...
    for model_name, model in model_pool.items():
        ...
        # Rebuild and optionally refit best pipeline on train+valid
        preprocess = _build_preprocess_pipeline(
            sample_normalization=sample_normalization,
            scaler=scaler,
            variance_threshold=variance_threshold,
    ...
    predictions_df = (
        pd.concat(artifacts.predictions, ignore_index=True) if artifacts.predictions else pd.DataFrame()
    )
    ...
    if not metrics_df.empty:
        plot_metric_comparison(
            ...
            metric="roc_auc",
            output_path=out_dir / "metric_comparison_roc_auc.png",
        )
        plot_metric_comparison(
            metrics_df=metrics_df,
            metric="f1",
            output_path=out_dir / "metric_comparison_f1.png",
        )
